...
connection.on("connect", (err) => {
  if (err) console.log("Error de conexión con la base de datos, intente nuevamente");
    ...
    connected = 1;
    helpers.deleteSala = async (sala) => {
      global.connected = 1;
      console.log("console numero:"+sala.numero);
      var data = [];
      var statment = "delete from cinema.dbo.Salas where numero=" + sala.numero + "";

      const request = await new tedious.Request(statment, function (err, rowCount, rows) {
        console.log("data sala: "+data[0])
        if (err) {
          data[0] = "No se pudo obtener la sala";
          console.log("data"+data[0])
        } else {
          data[0] = "Sala eliminada";
          console.log("data"+data[0])
        }
      });
